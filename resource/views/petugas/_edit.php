<?php if (isset($_POST['id'])): ?>
  ...
  <form class="was-validated" action="" method="post">
    ...
    <div class="form-group">
      ...
      <select class="form-control" name="level" id="level" required>
        <optgroup label="Pilih Level">
          <option value="<?= (empty($_POST['level'])) ? $rows->level_petugas: $_POST['level']; ?>" hidden><?= level((empty($_POST['level'])) ? $rows->level_petugas : $_POST['level']); ?></option>
          <?php for ($i=1; $i <= 4 ; $i++) { ?>
    ...
    <div class="form-group">
      ...
      <textarea class="form-control" required name="alamat" id="alamat"  rows="8" cols="80"><?= $rows->alamat_petugas; ?></textarea>
    </div>
    <div class="form-group">
  ...
  <script type="text/javascript">
    $(document).ready( function() {
      ...
      $('form').submit( function(e) {
        ...
        $.ajax({
          type: 'POST',
          url: 'resource/views/petugas/inc/_update.php',
          data: {
            id: $('#id').val(),
